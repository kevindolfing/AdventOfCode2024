...
public class Day6(InputAgent agent) : IDay
{
    ...
    private static void SimulateGuard(Map map)
    {
        ...
        while (!finished)
        {
            ...
            if (nextSquare.Item1 < 0 || nextSquare.Item1 >= mapList[0].Count || nextSquare.Item2 < 0 || nextSquare.Item2 >= mapList.Count)
            {
                finished = true;
                continue;
            }
    ...
    public async Task Part2()
    {
        ...
        List<Tile> considerPositions = map.Tiles.SelectMany(row => row.Where(tile => tile.Visited && (tile.X != originalStartingPosition.X || tile.Y != originalStartingPosition.Y))).ToList();
